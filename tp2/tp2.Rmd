...
df_palabras_ponderado <- df_tokens %>%
  group_by(word) %>%
  summarise(
    n_apariciones = n(),
    promedio_score = mean(imdb_score),
    sd_score = sd(imdb_score),
    q25 = quantile(imdb_score, 0.25),
    q75 = quantile(imdb_score, 0.75)
  ) %>%
  filter(n_apariciones >= 10) %>%
  mutate(ranking_score = promedio_score * log(n_apariciones + 1)) %>%
  arrange(desc(ranking_score)) %>%
  ungroup()

df_palabras_sin_ponderar <- df_tokens %>%
  group_by(word) %>%
  summarise(
    n_apariciones = n(),
    promedio_score = mean(imdb_score),
  ...
  filter(n_apariciones >= 10) %>%
  mutate(ranking_score = promedio_score) %>%
  arrange(desc(ranking_score)) %>%
  ungroup()
```
...
```{r}
top_palabras <- df_palabras_ponderado %>% slice_max(ranking_score, n = 25)

ggplot(top_palabras, aes(x = reorder(word, ranking_score), y = promedio_score)) +
...
```{r}
top_palabras <- df_palabras_sin_ponderar %>% slice_max(ranking_score, n = 25)

ggplot(top_palabras, aes(x = reorder(word, ranking_score), y = promedio_score)) +
  geom_col(fill = "steelblue") +
  coord_flip() +
  labs(title = "Top palabras asociadas a mayor puntaje IMDb (sin ponderar)",
       x = "Palabra", y = "Promedio IMDb") +
  theme_minimal()
```
...
bottom_palabras <- df_palabras_ponderado %>%
  slice_min(ranking_score, n = 25)

ggplot(bottom_palabras, aes(x = reorder(word, -ranking_score), y = promedio_score)) +
...
```
```{r}
bottom_palabras <- df_palabras_sin_ponderar %>%
  slice_min(ranking_score, n = 25)
