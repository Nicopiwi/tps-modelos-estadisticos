...
Dado que al hacer el primer análisis exploratorio nos dimos cuenta que había valores de edad fuera de rango, sacamos los outliers del dataset para tener datos representativos. Luego de ello, realizamos la separación train-test en proporción 80/20.

```{r, warning=FALSE, message=FALSE, results='hide'}
set.seed(123)

df <- read.csv2("C:/Users/a433312/Downloads/data.csv", sep="\t") |> 
  as.data.frame()
...
$\beta = -0.024$ lo que sugiere que al aumentar la edad, la probabilidad de estar "De acuerdo" disminuye.
...
En estos ejercicios haremos una comparación de modelos entre:

- Modelo Lineal Truncado: Corremos un modelo lineal "común" y truncaremos los resultados al entero más cercano del intervalo [1,5]


- Modelo Ordinal: Explicado inicialmente

La metrica de comparación será el MAE. La usaremos en el conjunto de test.
...
Habiendo visto estos resultados, es posible que esté ocurriendo alguna de estas dos opciones: o bien, hay un desbalance entre clases o la edad no es un buen predictor de 'Q30'.
...
En el anexo 1, exploramos las demás preguntas para averiguar si en alguna de ellas la edad explicase un poco mejor la varianza de las respuestas. Esto con el objetivo de ver si con las métricas del modelo ordinal se obtendrían mejores resultados que en el modelo lineal truncado.

### Ejercicio 10

En este ejercicio, crearemos un conjunto un modelos bayesianos de regresión ordinal con la librería `stan_polr`, variando la distribución prior del $R^2$ del parámetro $\beta$ y compararemos las distribuciones a posteriori.

Decidimos hacer un subsampleo a 5000 observaciones para mejorar los tiempos de ejecución del algoritmo MCMC. Considerando que este es un tamaño representativo de la población para hacer inferencia. 
 
```{r, warning=FALSE, message=FALSE, results='hide'}
where <- c(0.0001, 0.001, 0.005, 0.1, 0.9)

# Submuestreo estratificado
df_strat <- df_train_30 %>%
  group_by(Q30) %>%
  sample_frac(size = 5000 / nrow(df_train_30)) %>%
  ungroup()

# Lista para guardar las densidades
densidades <- list()
...
densidad_beta <- density(posterior$age)
...
densidades[[length(densidades) + 1]] <- df_densidad
...
df_densidades <- bind_rows(densidades)

ggplot(df_densidades, aes(x = x, y = y, color = R2)) +
  geom_line(size = 1) +
  labs(
    title = "Curvas de densidad del coeficiente de edad",
    x = "Beta (edad)",
    y = "Densidad"
  ) +
  theme_minimal() +
...
```
Se pude observar que para priors del $R^2$ donde su locación estaba centrada en valores muy cercanos a 0, la distribución a posteriori difiere considerablemente de las distribuciones a posteriori cuando se setearon los priors centrados en 0.1, 0.9 y Uniforme. 

Para valores muy bajos de prior_R2, prácticamente todo el comportamiento de la variable respuesta debería explicarse por el ruido, y casi nada por los predictores. Eso hace que los priors sobre los betas sean muy estrechos, centrados en cero. Por eso, los posteriors de los betas quedan muy "aplastados" y lejos del valor que te daría el likelihood solo (o sea, de la info que aporta la data).

A partir de valores como 0.005, 0.1 y 0.9, el prior ya no está restringiendo tanto los betas. Y como los datos contienen suficiente información, el likelihood domina, y el posterior ya no cambia tanto con distintas configuraciones del prior.
